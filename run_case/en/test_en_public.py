...
class TestPublic(object):
    ...
    def test_bottom_1(self):
        ...
        add1 = 'Unit 1401,14th floor,B20, Software Park III, NO.63 Chengyi North Street,Xiamen City'
        if xm_addr != add1 or xm_addr is None:
            tool.raise_and_screen(self.driver, '疑似公司地址错误')

        add2 = 'Room122,1st floor,NO.838 Guangji Street,Shanghai City'
        if sh_addr != add2 or sh_addr is None:
            tool.raise_and_screen(self.driver, '疑似上海地址错误')

        if not tool.wait_until_true(tool.is_img_loaded, 10, self.driver, sc_img)\
                or not tool.wait_until_true(tool.is_element_visible, 10, sc_img):
