...
import pageModule.en.en_home as home
...
if env == "ot":
    ...
    application_url = 'https://ot-www.sucheon.com/en/application.html'
else:
...
class TestHome(object):
    ...
    def test_header_3(self):
        ...
        time.sleep(1)
        video = self.home.get_video().is_displayed()
        if video:
            tool.raise_and_screen(self.driver, '点击播放前视频显示异常')
        self.home.get_banner_2_btn().click()
        time.sleep(5)
        video2 = self.home.get_video().is_displayed()
        if not video2:
    ...
    def test_header_4(self):
        ...
        # 跳转后应该能看到招聘详情
        employ = tool.wait_ele_be_founded(self.driver, 10, *(By.XPATH, '//*[@id="active"]/div/div[1]/span'))
        if not employ.is_displayed():
...
if __name__ == '__main__':
    ...
    pytest.main(['-s', './test_en_home.py', '--alluredir', './allure-results'])
