...
class TestNews(object):
    ...
    def test_header_1(self):
        ...
        if txt.text is None or tool.wait_until_true(tool.is_img_loaded, 10, self.driver, img) is False:
    ...
    @allure.description('查看全部新闻tab页的新闻是否显示正常')  # 用例描述
    @allure.title('全部模块测试')  # 用例标题
    def test_all_news_1(self):
        # 重新跳转到最新动态页
        self.driver.get(news_url)
        # 先滚动到底部 让新闻全部加载完成
        ...
        for n in range(all_news):
            # 获取卡片
            card = tool.wait_ele_be_founded(self.driver, 10, *(By.XPATH, self.news.news_cards[1]+f'[{n+1}]'))
    ...
    @allure.feature(feature)
    @allure.story('企业资讯')  # 用例模块
    @allure.description('查看企业资讯tab页的新闻是否显示正常')  # 用例描述
    @allure.title('全部模块测试')  # 用例标题
    def test_etp_news_1(self):
        # 重新跳转到最新动态页
        self.driver.get(news_url)
        self.news.get_enterprise_btn().click()
        # 先滚动到底部 让新闻全部加载完成
        tool.scroll_to_bottom(self.driver)
        time.sleep(5)
        tool.scroll_to_top(self.driver)
    ...
    @allure.feature(feature)
    @allure.story('新闻动态')  # 用例模块
    @allure.description('查看新闻动态tab页的新闻是否显示正常')  # 用例描述
    @allure.title('全部模块测试')  # 用例标题
    def test_action_1(self):
        # 重新跳转到最新动态页
        self.driver.get(news_url)
        self.news.get_news_btn().click()
        # 先滚动到底部 让新闻全部加载完成
        tool.scroll_to_bottom(self.driver)
        time.sleep(5)
        tool.scroll_to_top(self.driver)
...
if __name__ == '__main__':
    ...
    pytest.main(['-s', './test_ch_news.py', '--alluredir', '../allure-results'])
